...
export default function RewardsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = rewardsRecord
      ? { ...initialValues, ...rewardsRecord, Evaluation, PottyLog }
      : initialValues;
    setGoodBehavior(cleanValues.goodBehavior);
    setCompletedActivities(cleanValues.completedActivities);
    setDesctiptionRewardGiven(cleanValues.desctiptionRewardGiven);
    setRewardDate(cleanValues.rewardDate);
    setRewardTime(cleanValues.rewardTime);
    setEvaluation(cleanValues.Evaluation);
    setCurrentEvaluationValue(undefined);
    setCurrentEvaluationDisplayValue("");
    setPottyLog(cleanValues.PottyLog);
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Rewards, idProp)
        : rewardsModelProp;
      setRewardsRecord(record);
      const EvaluationRecord = record ? await record.Evaluation : undefined;
      setEvaluation(EvaluationRecord);
      const PottyLogRecord = record ? await record.PottyLog : undefined;
      setPottyLog(PottyLogRecord);
    };
    queryData();
  }, [idProp, rewardsModelProp]);
  React.useEffect(resetStateValues, [rewardsRecord, Evaluation, PottyLog]);
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Rewards.copyOf(rewardsRecord, (updated) => {
              Object.assign(updated, modelFields);
              if (!modelFields.Evaluation) {
                updated.rewardsEvaluationId = undefined;
              }
              if (!modelFields.PottyLog) {
                updated.rewardsPottyLogId = undefined;
              }
            })
          );
          if (onSuccess) {
