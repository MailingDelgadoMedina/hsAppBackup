...
export default function MyStudentUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setClassess(cleanValues.classess ?? []);
  ...
  const [linkedClassess, setLinkedClassess] = React.useState([]);
  const canUnlinkClassess = false;
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const linkedClassess = record
        ? await Promise.all(
            (
              await record.classess.toArray()
            ).map((r) => {
              return r.classes;
            })
          )
        : [];
      setLinkedClassess(linkedClassess);
    };
    queryData();
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const classessToLinkMap = new Map();
          const classessToUnLinkMap = new Map();
          const classessMap = new Map();
          const linkedClassessMap = new Map();
          classess.forEach((r) => {
            const count = classessMap.get(getIDValue.classess?.(r));
            const newCount = count ? count + 1 : 1;
            classessMap.set(getIDValue.classess?.(r), newCount);
          });
          linkedClassess.forEach((r) => {
            const count = linkedClassessMap.get(getIDValue.classess?.(r));
            const newCount = count ? count + 1 : 1;
            linkedClassessMap.set(getIDValue.classess?.(r), newCount);
          });
          linkedClassessMap.forEach((count, id) => {
            const newCount = classessMap.get(id);
            if (newCount) {
              const diffCount = count - newCount;
              if (diffCount > 0) {
                classessToUnLinkMap.set(id, diffCount);
              }
            } else {
              classessToUnLinkMap.set(id, count);
            }
          });
          classessMap.forEach((count, id) => {
            const originalCount = linkedClassessMap.get(id);
            if (originalCount) {
              const diffCount = count - originalCount;
              if (diffCount > 0) {
                classessToLinkMap.set(id, diffCount);
              }
            } else {
              classessToLinkMap.set(id, count);
            }
          });
          classessToUnLinkMap.forEach(async (count, id) => {
            const classesStudentsRecords = await DataStore.query(
              ClassesStudents,
              (r) =>
                r.and((r) => {
                  const recordKeys = JSON.parse(id);
                  return [
                    r.classesId.eq(recordKeys.id),
                    r.studentsId.eq(studentsRecord.id),
                  ];
                })
            );
            for (let i = 0; i < count; i++) {
              promises.push(DataStore.delete(classesStudentsRecords[i]));
            }
          });
          classessToLinkMap.forEach((count, id) => {
            for (let i = count; i > 0; i--) {
              promises.push(
                DataStore.save(
                  new ClassesStudents({
                    students: studentsRecord,
                    classes: classesRecords.find((r) =>
